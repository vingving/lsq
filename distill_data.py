...
def getDistilData(teacher_model,
    ...
    if path == '' or save_mode:
        ...
        for i, gaussian_data in enumerate(dataloader):
            print('Gen Distilled data( %d / %d )'%(i+1, len(dataloader)))
            # sys.stdout.flush()
            # if i == num_batch:
            #     break
            ...
            for it in range(500):
                ...
                progress_bar(it, 500, '(%d / %d)'% (it, 500))

            if save_mode:
                torch.save(gaussian_data, '%s/distill_data_file_%05d.pt'%(path, i))
                print('%s/distill_data_file_%05d.pt saved'%(path, i))
        ...
        for f in file_list:
            if 'distill_data_file_' in f:
                refined_gaussian.append(torch.load(os.path.join(path, f)))
        print('%d distill_data_file loaded'%(len(refined_gaussian)))
